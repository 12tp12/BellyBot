...
module.exports = class ApiAI {
    ...
    constructor(apiAiConfig) {
        this._apiAiConfig = apiAiConfig;
        var apiaiOptions = {
            language: apiAiConfig.apiaiLang,
            requestSource: "BELLY"
        };

        this._apiaiService = apiai(apiAiConfig.apiaiAccessToken, apiaiOptions);
        this._sessionIds = new Map();
    }

    processMessage(req, res) {
        if (this._apiAiConfig.devConfig) {
        ...
        if (req.body && req.body.session && req.body.session.from && req.body.session.initialText) {
            ...
            if (messageText) {
                ...
                apiaiRequest.on('response', (response) => {
                    if (ApiAI.isDefined(response.result)) {
